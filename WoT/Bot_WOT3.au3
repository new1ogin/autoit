...
while 1
	...
	sleep(200)
	WinActivate($thwnd)
	ConsoleWrite(@hour&':'&@MIN&':'&@SEC&' '& " Клик на копку БОЙ " & @CRLF)
	...
	For $i=1 to 20
		...
		If ($WaitCPU/1.6)<$menuCPU or ($WaitMEM/1.1)<$menuMEM then
			_Wait()
		Else
			ConsoleWrite(@hour&':'&@MIN&':'&@SEC&' '& " Режим ожидания закончен " & @CRLF)
			ExitLoop
		Endif
	next
	sleep(3000)
...
Func _Wait_Battle()
	ConsoleWrite(@hour&':'&@MIN&':'&@SEC&' '& ' Начато ожидание окончания боя ' &@CRLf)
	$w=1
	$schet=0
	$timerWait=TimerInit()
...
Func _turn()
	...
	If $turn=1 Then
;~ 		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $turn = ' & $turn & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console

		If 9<$turnd<19 then
			$turnd+=1
;~ 					ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $turnd = ' & $turnd & ' turn='&$turn  & @CRLF) ;### Debug Console
		Else
			$turnd=10
		EndIf
	Else
...
;~ 	ConsoleWrite(' перед сменой направления '') : $turnd = ' & $turnd & ' turn='&$turn  & @CRLF)
	If 13<$turnd and 19>$turnd Then
		...
		Else
			...
			$turnd=10
		EndIf
;~ 				ConsoleWrite(' смена направления '') : $turnd = ' & $turnd & ' turn='&$turn  & @CRLF)
	EndIf
