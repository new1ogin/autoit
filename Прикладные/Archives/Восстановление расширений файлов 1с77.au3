...
Global $aBaseFiles[2][5], $iBaseFiles = 1 ;массив со значениями -  Имя файла | Размер заголовка | Размер записи
...
;~ 		Next
;~ 		_ArraySort($aTemp, 1)
;~ 		_ArrayDisplay($aTemp,"первый метод " & StringRight($Filelist1[$i],40))
...
;~ 			ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $Filelist2[$j] = ' & $Filelist2[$j] & @CRLF) ;### Debug Console
		...
		Next
		_ArraySort($aTemp, 1)
		_ArrayDisplay($aTemp, "второй метод " & StringRight($Filelist1[$i], 40))
	Next
...
;~ 			_ArrayDisplay($Findall)
			...
			For $f = 0 To UBound($Findall) - 1
				If $aBaseFiles[$Findall[$f]][2] = $Num2 And $aBaseFiles[$Findall[$f]][1] = $Num1 Then
					...
					Else
						$newName &= '+' & $aBaseFiles[$Findall[$f]][0]
						ConsoleWrite("Двойное имя = " & $newName & @TAB & "; Р.заг. = " & $aBaseFiles[$Findall[$f]][1] & @TAB & "; Р.зап. = " & $aBaseFiles[$Findall[$f]][2] & @CRLF)
					EndIf
					$tname[$itn][0]= $aBaseFiles[$Findall[$f]][0]
					$tname[$itn][1]= _compareZagolovki(_GetZagolovki($path & $Filelist[$i],1), $aBaseFiles[$Findall[$f]][4])
					$itn+=1
;~ 					;поиск имени исследуя загаловок
;~ 					$hft = FileOpen($aBaseFiles[$Findall[$f]][3]&$aBaseFiles[$Findall[$f]][0],16)
			...
			If $newName = '' Then
				...
				For $f = 0 To UBound($Findall) - 1
					ConsoleWrite("Имя = " & $aBaseFiles[$Findall[$f]][0] & @TAB & "; Р.заг. = " & $aBaseFiles[$Findall[$f]][1] & @TAB & "; Р.зап. = " & $aBaseFiles[$Findall[$f]][2] & @CRLF)
				Next
			Else
...
;~ 				ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $newName = ' & $newName & @CRLF) ;### Debug Console
				...
				;определение нужного имени из загловоков
...
;~ 					Exit
				...
				; добовление к имени цифры если файл существует
				$rename = ''
...
Func _compareZagolovki($Zag1,$Zag2)
	;считаем количество загаловков в строках
	$aZag1 = StringSplit($Zag1,'&')
	$countZag1 = $aZag1[0]
	$aZag2 = StringSplit($Zag2,'&')
	$countZag2 = $aZag2[0]
	If $countZag2 < $countZag1 Then ;меняем заголовки так, чтобы первый был наименьшим
		$t = $countZag1
		$countZag1 = $countZag2
		$countZag2 = $t
		$t = $Zag1
