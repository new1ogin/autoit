...
;~ 		_testMessage('@@ Debug(' & $switch & ') : $hwnd = ' & $hwnd & @CRLF) ;### Debug Console
		If $switch = 0 Then
			$switch = 1
			$hwnd = WinGetHandle("[TITLE:Сохранить отчёт; CLASS:#32770]")
			WinActivate($hwnd)
...
;~ 			_testMessage('@@ Debug(' & @ScriptLineNumber & ') : $sText = ' & $sText & @CRLF) ;### Debug Console
			If StringLen(StringStripWS($sText, 3)) > 1 Then ; проверка на получение любого текста
				If $sText = "Report" Then ; проверка на получение определенного текста, зачем шут знает...
	...
	;~ 					_testMessage('@@ Debug(' & @ScriptLineNumber & ') : $sText = ' & $sText & @CRLF) ;### Debug Console
			...
			Else
				$switch = 0
			EndIf
			; а теперь проверим, что что-то действительно изменилось
			Sleep(400 * $Kslp)
			$sText = ControlGetText($hwnd, "", "[CLASS:Edit; INSTANCE:1]")
			If $sText = "Report" Then $switch = 0
;~ 			_testMessage($sText & @CRLF)
		EndIf
	Else
		$switch = 0
	EndIf
...
;~ 			_testMessage('Подсказка - $fileList2 > $fileList '& $t & '&' & $fileList2 & '&' & $fileList2)
			RunWait($FileSendReportsOnFTP)
			$fileList = $fileList2
		EndIf
;~ 		_testMessage('Подсказка - Пробую найти отчеты в моих документах '& $t & '&' & $fileList2 & '&' & $fileList2)
		If FileExists($FilePathMyDocReports) Then
...
;~ 				_testMessage('Подсказка '& $t & '&' & $fileListDoc2 & '&' & $fileListDoc2)
				$fileReport = _GetYoungFile($FilePathMyDocReports)
				if $fileReport <> 0 Then
	...
	EndIf
	Sleep(200 * $Kslp)
;~ 	_testMessage('Подсказка - Проверка на длительность ожидания '& $timerDiff & '&' & $timerDiff & '&' & '')
	If $timerDiff > $TimeToWait Then ExitLoop
;~ 	_testMessage('Подсказка - Проверка пройдена '& $timerDiff & '&' & $timerDiff & '&' & '')
WEnd
...
Func _testMessage($text)
	TrayTip('Подсказка', $text, 1500)
	sleep(1000)
EndFunc
