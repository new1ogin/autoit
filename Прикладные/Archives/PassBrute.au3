...
;~ 		$t = FileReadLine($hf,$shet)
;~ 		If @error = -1 Then $exitloop=1
;~ 		Execute($t)
;~ 				ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : FileReadLine($hf,$shet) = ' & FileReadLine($hf,$shet) & @CRLF ) ;### Debug Console
;~ 		$shet+=1
;~ 		sleep(100)
...
;~ 	For $m = 1 To 12
;~ 		For $d = 1 To 30
;~ 			$aPass[$ip] = StringRight('0' & $d, 2) & StringRight('0' & $m, 2) & $y
;~ 			$ip += 1
;~ 		Next
;~ 	Next
;~ Next
