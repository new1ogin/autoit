...
Global $1CMarkerProcess = '1cv8c.exe'
...
Func _TestStartTime($path)
	...
	While 1
		...
		$procCPU = _ArrayMax($aCPU) - _ArrayMin($aCPU)
		If @error Or StringInStr($procCPU, '#IND') > 0 Then $procCPU = 0
		$procMEM = 100 * _ArrayMax($amemr) / _ArrayMin($amemr) - 100
...
Func _findPid1C($FindProc1)
	...
	If $FindProc2[0][0] <= $FindProc1[0][0] Then
		if ProcessExists($1CMarkerProcess) Then
			For $i = 0 To 1000
				$FindProc2 = ProcessList($1CTrackProcess)
				If $FindProc2[0][0] > $FindProc1[0][0] Then ExitLoop
				Sleep(100)
