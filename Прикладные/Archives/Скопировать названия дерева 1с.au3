...
$filet = @ScriptDir & '\TextFromTreeTemp.txt'
ClipPut('')
Global $ide = 1
...
;~ 		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $ide = ' & $ide & @CRLF) ;### Debug Console
	...
	If $tcount[0] <= 2 Then
		; попытка выйти из длинной очереди повтор€ющихс€ заголовков
		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : попытка выйти из длинной очереди повтор€ющихс€ заголовков = ' & $tcount[0] & @CRLF) ;### Debug Console
		Send('{Down}')
		Send('{Down}')
	EndIf
WEnd
...
Func _SaveThisElm($Clip)
	...
	FileWriteLine($filet,$tabs & $Clip & @CRLF)
