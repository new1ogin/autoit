...
;~ 			For $r=0 to UBound($iReturn)-1
			...
			For $f = 0 to UBound($Findall)-1
				if $aReturn[$Findall][2] = $Num2 Then
					if $newName = '' Then
						$newName = $aReturn[$Findall][1]
					Else
						$newName &= '|' & $aReturn[$Findall][1]
					EndIf
				EndIf
			...
			if $newName = '' Then
				...
				For $f = 0 to UBound($Findall)-1
					ConsoleWrite("Имя = "&$aReturn[$Findall][0]&@TAB&"; Р.заг. = "&$aReturn[$Findall][1]&@TAB&"; Р.зап. = "& $aReturn[$Findall][2] & @CRLF)
				Next
			Else
				FileMove($path & $Filelist[$i], $path & $newName)
			EndIf
