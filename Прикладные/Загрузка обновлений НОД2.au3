...
;~ 	_ArrayDisplay($aArchivList)
	...
	EndIf
EndIf
ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $ArchivList = ' & $ArchivList & @CRLF) ;### Debug Console
$timerText= Timerinit()
;~ _Archivate($folder&'eset_upd\update.ver',$folder) ;Создание общего архива обновлений
...
if Ubound($aArchivList) > 1 then
	for $i=1 to $aArchivList[0][0]
		if StringInStr($aArchivList[$i][0],'update.ver') > 0 Then
			...
			EndIf
		EndIf
		if StringInStr($aArchivList[$i][0],'mpam-fe.exe') > 0 Then
			if StringInStr($aArchivList[$i][3] ,StringReplace(_NowDate(),'.','/')) > 0 Then
				$ArchivateM=0
			EndIf
		EndIf
		if StringInStr($aArchivList[$i][0],'mpam-fex64.exe') > 0 Then
			if StringInStr($aArchivList[$i][3] ,StringReplace(_NowDate(),'.','/')) > 0 Then
				$ArchivateM64=0
			EndIf
		EndIf
...
if not FileExists($folder&$name[6]&'_'&_NowDate()&'.'&$name[7]) then
	...
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $DownloadBase = ' & $DownloadBase & @CRLF) ;### Debug Console
...
if StringInStr($ArchivList,$name[6]&'_'&_NowDate()&'.'&$name[7]) < 1 and FileExists($folder&$name[6]&'_'&_NowDate()&'.'&$name[7]) Then
	FileMove ( $folder&$name[6]&'_'&_NowDate()&'.'&$name[7], $folder&'eset_upd\'&$name[6]&'.'&$name[7], 1)
	_Archivate($folder&'eset_upd\'&$name[6]&'.'&$name[7],$folder) ;Создание общего архива обновлений
	FileMove ( $folder&'eset_upd\'&$name[6]&'.'&$name[7], $folder&$name[6]&'_'&_NowDate()&'.'&$name[7], 1)
EndIf
_DeleteOldFiles($folder,$name[6] & '_' & "*." & $name[7] ,2) ;Удаление лишних файлов
...
if $SwitchSfxNeed = 1 or not FileExists($folder&'AllUpdates4Main.exe') or _WinAPI_GetFileSizeEx($hFile) < 1048576 Then
	...
	RunWait(@ComSpec & " /c " & 'COPY /b '&$folder&'7zsd_LZMA.sfx + '& _
...
Func _Archivate($patch,$folder)
	...
	if FileExists($patch) <> 1 then
		ConsoleWrite('(' & @ScriptLineNumber & ') Exit')
		Exit
	EndIf
...
;~ 	EndIf
	if FileExists($folder&'AllUpdates4Main'&'_'&_NowDate()&'.7z') > 0 then
		$SwitchSfxNeed = 1
		$7ZipUpdate = _7ZipUpdate(0, $folder&'AllUpdates4Main'&'_'&_NowDate()&'.7z', $patch, " -hide")
;~ 		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $7ZipUpdate = ' & $7ZipUpdate & @CRLF) ;### Debug Console
	Else
		$SwitchSfxNeed = 1
		$7ZipAdd = _7ZipAdd(0, $folder&'AllUpdates4Main'&'_'&_NowDate()&'.7z', $patch, " -hide")
