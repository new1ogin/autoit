...
Dim $CmdLineN[2]
$CmdLineN[0]=1
$CmdLineN[1]=@scriptdir&'\Пароли.txt'
...
ConsoleWrite('НАЧАЛО ПОДБОРА ' & @CRLF)
$TempSleep = 10
$StepSleep = 1000
For $i = 1 To $aPasswords[0]
	ClipPut($aPasswords[$i])
...
Exit
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $aPasswords[0] = ' & $aPasswords[0] & @CRLF) ;### Debug Console
...
Func _NumGenerator($num)
	...
	For $i = 2 To $num
		$nulls &= '0'
	Next
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $nulls = ' & $nulls & @CRLF) ;### Debug Console
	For $i = 1 To (10 ^ $num)
...
;~ 	return $array2
	...
	Local $add[14] = ['15', '12', '13', '11', '10', '14', '16', '2015', '2012', '2013', '2011', '2010', '2014', '2016']
	Local $array3[($array2[0] + 1) * (UBound($add) + 1)]
	For $a = 1 To $array2[0] ;добовляем сами обработанные пароли
		$array3[$a] = $array2[$a]
	Next

	For $i = 0 To UBound($add) - 1 ; добовляем года к паролям
		For $a = 1 To $array2[0]
			$t = StringRegExpReplace($array2[$a], '(.*?[^0-9])[0-9]{2,4}$', '$1') ; отчищяем пароль от тех годов которые в нём уже есть
			$t = $t & $add[$i]
			If $t <> $array2[$a] Then
				$array3[$array2[0] * ($i + 1) + $a] = $t
;~ 				ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $array2[0] * ($i + 1) + $a = ' & $array2[0] * ($i + 1) + $a & @CRLF) ;### Debug Console
			EndIf
		Next
	Next

	_UniqueArrayNear($array3, 0, 1)
	$array3 = _ArrayClearEmpty($array3)
