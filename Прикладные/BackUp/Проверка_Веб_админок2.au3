...
$CloseOtherWindows = 1; Закрывать все окна кроме IE
...
$titleWinList = 'Безопасность Windows|Подключение к|Windows Security'
...
Func _CheckWinPass($iadd)
	...
	If $aStrings[$iadd][1] = '' Then
		...
		ConsoleWrite('Содержимое страницы неправильного ответа: ' & StringLen($aStrings[$iadd][1]) & @CRLF)
;~ 		_IEStartCheck($iadd)
;~ 		Return 1
		_reloadPage($iadd)
	EndIf
	...
	For $p = 0 To 0;UBound($aLoginPass) - 1 Step $CountTestPass ; цикл пока не закончатся все пароли
		...
		Sleep($sleep)
		If $p > UBound($aLoginPass) - 3 Then ContinueLoop ;Костыль как то связанный с tep $CountTestPass
		_reloadPage($iadd)
...
Func _CloseAllWindow()
	...
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $hWin = ' & $hWin & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	$t = _ArrayCompareTwo($aWinList, $aWinListOld)
	_ArrayDisplay($t)
	$t = _ArrayCompareTwo($aWinListOld, $aWinList)
	_ArrayDisplay($t, 'DVA')

	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : @AutoItExe = ' & @AutoItExe & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console


EndFunc   ;==>_CloseAllWindow
...
Func _ArrayCompareTwo(Const ByRef $a1, Const ByRef $a2, $dim = 1)
	If Not IsArray($a1) Or Not IsArray($a2) Then Return
	Local $sUniq[1][UBound($a1, 2)], $schet = 0
	For $i = 0 To UBound($a1) - 1
		For $j = 0 To UBound($a2) - 1
			If $a1[$i][$dim] = $a2[$j][$dim] Then ExitLoop
		Next
		If $j = UBound($a2) Then
			ReDim $sUniq[$schet + 1][UBound($a1, 2)]
			For $d = 0 To UBound($a1, 2) - 1
				$sUniq[$schet][$d] = $a1[$i][$d]
			Next
			$schet += 1
		EndIf
	Next
	Return $sUniq
EndFunc   ;==>_ArrayCompareTwo
