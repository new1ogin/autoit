...
Global $Form1 = 0, $paus = 0, $exitFunc = 0, $startFunc = 0, $workFunc = 0
...
$iMenu = TrayCreateItem("Остановить и показать Меню")
...
Func clr()
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : clr() = ' & @CRLF) ;### Debug Console
EndFunc   ;==>clr
...
Func _GuiCreate()
	...
	While 1
		...
		Switch $nMsg
			...
			Case $ButtonArea
				if _startFromGUI('_Area()')=1 then ExitLoop
			Case $ButtonProc
			Case $Label8
			Case $Label9
			Case $Label10
