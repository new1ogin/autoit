...
HotKeySet("{F7}", "_Exit")
Global $Paused, $oIE, $hLog, $OnlySearch, $sleep, $sleepMin, $sleepLoad, $CountTestPass,$hFileDone,$hFileFindWin,$hFileFindWeb,$aDone
...
Local $Content, $Strings, $IgnorWordList, $WhiteList, $IgnorRegExpList,$timer,$q=0
...
For $i = 1 To $Strings[0]
	$timer = TimerInit()
	if _ArrayBinarySearch($aDone,$Strings[$i],1) = -1 Then ; проверка не выполнялся ли уже этот адрес
		_IEStartCheck($Strings[$i]);Запуск проверки адреса в браузере
...
;~ 	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $a = ' & $a & @CRLF) ;### Debug Console
	...
	If $search = 1 And StringLen($html) > 100 Then
		_Log_Report($hLog, 'Найден контент на сайте: ' & $address & @CRLF)
