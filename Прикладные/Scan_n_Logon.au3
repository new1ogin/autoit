...
$hLog = _Log_Open(@ScriptDir & '\Scan_n_Logon.log', '### Лог программы Scan_n_Logon ###')
...
;~ 	_Log_Report($hLog, "Завершение запуска... Результат: " & @TempDir & '\' & $randstr & '.rdp' & ' PID mstsc: ' & $Pidmstsc & ' ' & @CRLF)
	Dim $return[2]
	$return[0] = $randstr
	$return[1] = $r
	return $return
