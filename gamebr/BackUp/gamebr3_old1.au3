...
#include <Color.au3>
global $aColorGems[5][3] = [[87, 74, 54],[214, 196, 148],[99, 172, 57],[192, 153, 24],[124, 57, 66]]
global $x1, $y1, $C
...
Func _DetectAndClick()
		$aPos = _GetArrayLocation($x1, $y1, $x2, $y2, $H, $W, $C)
	;~ 	;вывод в консоль
	;~ 	For $i = 0 To 5
	;~ 		For $j = 0 To 5
...
Func _ClickChoise($i,$j,$h)
	...
	if $h = 0 Then
		_mouseClick($x1 + ($j * $C + ($C / 2)), $y1 + (($i + 1) * $C + ($C / 2)))
	Else
		_mouseClick($x1 + (($j + 1) * $C + ($C / 2)), $y1 + ($i * $C + ($C / 2)))
	EndIf
Endfunc
...
Func _GetArrayLocation($x1, $y1, $x2, $y2, $H, $W, $C)
	...
	For $y = 0 To $H / $C - 1
		...
		$Matrix[$y][$x] = _CompareColor($aColors)
...
Func _CompareColor($aColors)
		local $aCompare[5]
		For $col=0 to 4
			For $nc=0 to 2
				$aCompare[$col]+=Abs($aColors[$nc]-$aColorGems[$col][$nc])
			Next
		Next
		return _ArrayMinIndex($aCompare)
EndFunc
