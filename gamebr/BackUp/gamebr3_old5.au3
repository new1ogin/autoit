...
_CopareImage($xnb+23,$ynb+74,$xnb+709,$ynb+454,5,5)
Exit
Func _CopareImage($xstart,$ystart,$xend,$yend,$qx,$qy)
   if Ubound($compImage) < $qx then
	  Redim $compImage[$qx][$qy][3]
   EndIf

   $timer=TimerInit()
   For $i=0 To $qx-1
	   For $j=0 to $qy-1
		   $t = PixelChecksum($xstart*$shag,$ystart*$shag,$xstart*$shag+$shag,$ystart*$shag+$shag)
