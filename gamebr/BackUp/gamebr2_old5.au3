...
;~ 								If $i2 <> 0 And $aPosC2[$i2 - 1][$j2][$C] = 1 Then $q += 1
								...
								if _CompareWithArray($aRightChange, $i, $j, $H, -1, -1, $c, 0) > -1 then $qs = ''
								ConsoleWrite('(' & $h & ')(' & $i & ')(' & $j & ')($i2=' & $i2 & ')($j2=' & $j2 & ')(' & $qs & '): по i ' & '' & @CRLF) ;### Debug Console
							EndIf
...
;~ 								If $j2 <> 0 And $aPosC2[$i2][$j2 - 1][$C] = 1 Then $q += 1
								...
								$qs &= "+" & $q
								;проверка не была ли часть этого совпадения уже найдена
								if _CompareWithArray($aRightChange, $i, $j, $H, -1, -1, $c, 1) > -1 then $qs = ''
								ConsoleWrite('(' & $h & ')(' & $i & ')(' & $j & ')($i2=' & $i2 & ')($j2=' & $j2 & ')(' & $qs & '): по j ' & '' & @CRLF) ;### Debug Console
							EndIf
