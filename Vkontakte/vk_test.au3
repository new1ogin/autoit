...
global $ShingleLen = 7
global $MonNames = 'янв|фев|мар|апр|мая|июн|июл|авг|сен|окт|ноя|дек', $aMonNames
dim $aMonNames[12] = ['янв', 'фев', 'мар', 'апр', 'мая', 'июн', 'июл', 'авг', 'сен', 'окт', 'ноя', 'дек']
...
For $i=0 to (UBound($aArticles)-1) step 6
	...
	$date = $aArticles[$i+4]
	$other = $aArticles[$i+5]
	if StringInStr($date,'сегодня') then
		$aTemp = StringRegExp($date,'сегодня в (\d+):(\d+)',2)
;~ 		$date = @YEAR&'/'&@MON&'/'&@MDAY&' '&StringRight('0'&$aTemp[1],2)&':'&StringRight('0'&$aTemp[2],2)&':'&'00'
	...
	Elseif StringInStr($date,'вчера') then
		$aTemp = StringRegExp($date, 'вчера в (\d+):(\d+)',2)
;~ 		$date = @YEAR&'/'&@MON&'/'&@MDAY&' '&StringRight('0'&$aTemp[1],2)&':'&StringRight('0'&$aTemp[2],2)&':'&'00'
		$date = StringFormat("%04d/%02d/%02d %02d:%02d:%02d", @YEAR, @MON, @MDAY, $aTemp[1], $aTemp[2], 0)
		$date = _DateAdd('D', -1, $date)
	Elseif StringRegExp($date,'(\d+) ('&$MonNames&') в (\d+):(\d+)') Then
		$aTemp = StringRegExp($date,'(\d+) ('&$MonNames&') в (\d+):(\d+)',2)
		for $m=0 to 11
...
;~ 		$date = @YEAR&'/'&$mon&'/'&StringRight('0'&$aTemp[1],2)&' '&StringRight('0'&$aTemp[3],2)&':'&StringRight('0'&$aTemp[4],2)&':'&'00'
	...
	Elseif StringRegExp($date,'(\d+) ('&$MonNames&') (\d{4})') Then
		...
		for $m=0 to 11
			...
			EndIf
		Next
		;присваиваем время учитывающее порядок, но не отрожающее время
