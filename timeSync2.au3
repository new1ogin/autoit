...
#AutoIt3Wrapper_Outfile=timeSync2(add12delay15).exe
...
;~         ShellExecute(@AutoItExe, $CmdLineRaw, @WorkingDir , "runas", @SW_SHOWNORMAL)
		RunAsWait($sUserName, @ComputerName, $sPassword, 0, '"'&@AutoItExe&'"', @WorkingDir)
        ; 1) путь к файлу
        ; 2) параметры
        ; 3) рабочий каталог
...
EndFunc   ;==>NTP_Connect
Func Set_Time($bdata)
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $bdata = ' & $bdata & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	...
	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $value = ' & $value & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	if $value < 100 Then
		msgbox(0,"Ошибка","Ошибка синхронизации времени",5)
		Exit
	EndIf
...
;~ 		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $NTP_IP = ' & $NTP_IP & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
        ...
        If $trig = 0 Then
			ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : Ubound($adata) = ' & Ubound($adata) & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
            $UTC = Set_Time($adata)
