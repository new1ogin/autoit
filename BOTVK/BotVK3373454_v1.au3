...
HotKeySet("{F1}", "_DetectWindow_and_start2") ;собирает вещи на первой локации
...
Func pause()
While 1
   if Guictrlread($CheckBox1)=1 then Traytip ("Подсказка","Пауза",100)
    Sleep(10000)
WEnd
EndFunc   ;==>pause
...
$Form1_2 = GUICreate("Бот на Мертвая зона в ВК", 362, 442, -1, -1, BitOR($GUI_SS_DEFAULT_GUI,$WS_SIZEBOX,$WS_THICKFRAME))
$sleepclick = GUICtrlCreateInput("1000", 232, 312, 121, 21)
$Label3 = GUICtrlCreateLabel("Введите задержку между кликами в мс.", 8, 312, 210, 17)
$Bossnumber = GUICtrlCreateCombo("Хохотун", 208, 344, 145, 25, BitOR($CBS_DROPDOWN,$CBS_AUTOHSCROLL))
GUICtrlSetData(-1, "Близняшко|Мясник")
$Label4 = GUICtrlCreateLabel("Выберите Босса для группового боя", 8, 344, 190, 17)
$WorkEnergynumber = GUICtrlCreateCombo("Стройка", 208, 376, 145, 25, BitOR($CBS_DROPDOWN,$CBS_AUTOHSCROLL))
GUICtrlSetData(-1, "Не использовать")
$Label5 = GUICtrlCreateLabel("Место для использования энергии", 8, 376, 182, 17)
$Button1 = GUICtrlCreateButton("Выход", 143, 408, 105, 25)
;~ $Button1 = GUICtrlCreateButton(GUICtrlRead($sleepclick), 143, 408, 105, 25)
GUICtrlSetOnEvent(-1, "_Pro_Exit")
$Checkbox1 = GUICtrlCreateCheckbox("Показывать всплывающие подсказки?", 8, 288, 241, 17)
GUICtrlSetState(-1, $GUI_CHECKED)
GUICtrlCreateEdit("", 8, 8, 329, 177, BitOR($ES_AUTOVSCROLL,$ES_AUTOHSCROLL,$ES_READONLY))
GUICtrlSetData(-1, StringFormat("Откройте приложение в отдельнои окне (не развёрнутом), \r\nперейдите на вторую локацию и нажмите клавишу "&Chr(34)&"F2"&Chr(34)&"\r\nЭто окно вы можете просто свернуть.\r\nНа данный момент бот выполняет следующие функции:\r\n1. Бой с появившимися на 1 и 2 локации громилами, зомби\r\nи сбор предметов\r\n2. Принятие участия в групповых боях и нападениях на босов\r\n3. Выполняет работы использую энергию\r\n"&Chr(34)&"F3"&Chr(34)&" - Покажет в подсказке на какой вы локации\r\n"&Chr(34)&"F4"&Chr(34)&" - Пауза, необходимо перезапустить на "&Chr(34)&"F2"&Chr(34)&"\r\n\r\nЕсли хотите изменить размер окна, то делайте это так, чтобы\r\nверхний угол приложения был виден"))
GUISetState(@SW_SHOW)
...
$Pic = GUICtrlCreatePic("", 8, 165+24, 204, 82)
...
Traytip('',$sleepD & '   '&$sleepH& '   '&$EndableH,100)
;~ sleep(1000)
