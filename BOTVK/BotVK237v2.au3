...
global $hWnD, $sControl, $xcor, $ycor
...
if @HotKeyPressed="{F2}" then global $loc=2
   $getcolorL=Hex(PixelGetColor ( 776+$xcor , 133+77+$ycor , $hWnd ), 6)
   if $getcolorL='615B4D' then global $loc=1
   if $getcolorL='171C08' then global $loc=2
	  TrayTip ("Подсказка",'Вы на' & $loc & 'локации', 100)
   if $Targetloc=0 then
	  Return
   Else
