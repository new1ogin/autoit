...
;~ 			MouseMove ( $cx, $cy+77 ,0 )
;~ sleep(64)
   ControlClick ($hWnD, '',$sControl, "left", 1, $cx+random(-4,4,1), $cy+random(-4,4,1))
...
Func _doit()
   ...
   ;определение локации
if @HotKeyPressed="{F1}" then global $loc=1
if @HotKeyPressed="{F2}" then global $loc=2
...
;~ 		 $B_Close = 0
;~ 		 $B_Close = PixelSearch ( 460, 560+77, 470, 570+77, 0x6EAD55,1,1, $hWnd )
;~ 	  if $B_Close<>0 then 
...
;~ 	$getcolor_attack=Hex(PixelGetColor ( 358+$xcorrect , 442+77 , $hWnd ), 6)
...
func _fight()
   ...
   _Click($cx_attack, $cy_attack)
	...
	if TimerDiff($hTimer_f) > 600000 then return ;замер времени на случай ошибки и "вечного боя"
			 ...
			 ;закрывает сообщение о победе
		 $B_Close = 0
		 $B_Close = PixelSearch ( 460, 560+77, 470, 570+77, 0x6EAD55,10,1, $hWnd )
	  if $B_Close=0 then 
		 _fight()
		 else
		 _Click($B_Close[0], $B_Close[1]-77)
...
;~    
	  for $i=0 to 2 ;Подтверждает выбор боя ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ
		 $B_Select = 0
		 $B_Select = PixelSearch ( $px1, $py1+77, $px2, $py2+77, 0x6EAD55,$variation,1, $hWnd )
		 if $B_Select=0 then 
			$i=0
			else
			_Click($B_Select[0], $B_Select[1]-77)
...
;~ 	  if @MIN=14 or @MIN=59 or @MIN=29 or @MIN=15 or @MIN=00 or @MIN=30 or @MIN=13 or @MIN=58 or @MIN=28 and $wait_for_battle=1 then _fight_detect()
		  if @MIN=14 or @MIN=59 or @MIN=29 or @MIN=13 or @MIN=58 or @MIN=28 and $wait_for_battle=1 then _fight_detect()
EndFunc
...
;~ 		 _Click(614, 145) ;выбор мясорубки
	  ...
	  for $i=0 to 63
		 $coord = PixelSearch( 575, 310+77, 580, 700+77, 0x6EAD55, 5 )
		 If Not @error Then
			 _Click($coord[0], $coord[1]-77)
			 sleep(6000)
		  Else		 
			...
			$B_Close = 0
			$B_Close = PixelSearch ( 520, 420+77, 525, 700+77, 0x6EAD55,1,1, $hWnd )
		 if $B_Close<>0 then _Click($B_Close[0], $B_Close[1]-77)
			 ExitLoop
		  EndIf
...
;~ 	  for $i=0 to 2 ;Подтверждает выбор боя ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ;ДОБАВИТЬ СЛУЧАЙ ОТСУТСТВИЯ КНОПКИ
;~ 		 $B_Select = 0
;~ 		 $B_Select = PixelSearch ( 565, 400+77, 720, 415+77, 0x6EAD55,10,1, $hWnd )
;~ 		 if $B_Select=0 then 
;~ 			$i=0
;~ 			else
;~ 			_Click($B_Select[0], $B_Select[1]-77)
...
;~ 		 $B_Select = 0
;~ 		 $B_Select = PixelSearch ( 325, 550+77, 495, 565+77, 0x6EAD55,10,1, $hWnd )
;~ 			if $B_Select=0 then 
;~ 			   $i=0
;~ 			   else
;~ 			   _Click($B_Select[0], $B_Select[1]-77)
