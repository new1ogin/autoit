...
$PIDparent = _WinAPI_GetParentProcess()
$NameProcParent = _WinAPI_GetProcessFileName($PIDparent)
...
;~ 		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : StringinStr($AutoItExe,''#'') = ' & StringinStr($AutoItExe,'#') & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
	if StringinStr($AutoItExe,'#')<>0 then
		$GetAdress = StringRegExp($AutoItExe,'#([A-Za-z0-9._-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4})',1)
	Else
		$GetAdress = StringRegExp($AutoItExe,'([A-Za-z0-9._-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4})',1)
	EndIf
