...
$minCmdLines = 4
$maxCmdLines = 6
$Commands = 'address|user|pass'
$helptext = "Функция проверки запуска службы spooler по Radmin. Поместить в папку с Radmin.exe" & @CRLF & _
		"Использование: " & @ScriptName & " -address address:port -user USERNAME -pass PASSWORD" & @CRLF & _
		"Пример: " & @ScriptName & " 192.168.0.190:4899 -user User -pass QWErty123" & @CRLF
_GetCMDParams($Commands,$minCmdLines,$maxCmdLines,$helptext)
Func _GetCMDParams($Commands,$minCmdLines,$maxCmdLines,$helptext)
		...
		For $i = 1 To $CmdLineN[0]
			For $c = 1 To $aCommands[0]
				If $aCommands[$c] = StringMid($CmdLineN[$i], 2, -1) Then
					Assign($aCommands[$c], $CmdLineN[$i + 1], 2)
					$i += 1
					ExitLoop
				EndIf
			Next
...
ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $pass = ' & $pass & @CRLF) ;### Debug Console
ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $user = ' & $user & @CRLF) ;### Debug Console
exit
