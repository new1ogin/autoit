...
#AutoIt3Wrapper_Outfile=ChangeMac(m010203040506).exe
#EndRegion ;**** Directives created by AutoIt3Wrapper_GUI ****

#include <GUIConstantsEx.au3>

Global $OSver = @OSVersion
If $OSver = 'WIN_7' then $OSver = "WIN_VISTA"
...
EndIf
$mymac=_GetMAC()
$TestFileName = 'ChangeMac(m010203040506).exe'
$defmac = _GetParamsOfFileName('Mac','m','',0)
ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $defmac = ' & $defmac & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
if $defmac <> $mymac then
	FileCopy ( @AutoItExe, @ScriptDir & '\ChangeMac(m'&$mymac&').exe')
	If $defmac = '' then $defmac = $mymac
EndIf
...
$newMAC = GUICtrlCreateInput( $defmac, 10, $i2 + 50, 150, 20)
...
GUISetState(@SW_SHOW)
;~ Sleep(3000)
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $newMAC = ' & $newMAC & @CRLF ) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $toggle = ' & $toggle & @CRLF ) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $ok = ' & $ok & @CRLF ) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $revertOld = ' & $revertOld & @CRLF ) ;### Debug Console
;~ $newMAC = GUICtrlRead($newMAC)
;~ $toggle = GUICtrlRead($toggle)
;~ $ok = GUICtrlRead($ok)
;~ $revertOld = GUICtrlRead($revertOld)
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $newMAC = ' & $newMAC & @CRLF ) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $toggle = ' & $toggle & @CRLF ) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $ok = ' & $ok & @CRLF ) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $revertOld = ' & $revertOld & @CRLF ) ;### Debug Console
...
Func _GetParamsOfFileName($ParamsName, $ParamsShortName = '', $Default = 0, $textMode=0)
	...
	If $textMode=0 then
		$params = StringRegExp($ScriptFileName[6],'\(.*?' & $ParamsName & '([0-9A-F.]+).*?\)',2)
		if @error then $params = StringRegExp($ScriptFileName[6],'\(' & $ParamsShortName & '([0-9A-F.]+).*?\)',2)
		if @error then $params = StringRegExp($ScriptFileName[6],'\(.*?\d' & $ParamsShortName & '([0-9A-F.]+).*?\)',2)
		if @error then
			Dim $params[2]
			$params[1] = $Default
