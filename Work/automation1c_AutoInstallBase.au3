...
$slp=3
$Startchange = 1; InputBox(" Выбор "," Введите номер певого запускаемого окна. 1 - 1С Предприятие ; 2 - 1С Конфигуратор ",1)
$CloseWinPp=0
$timeoutStart=0
$timerExitPp=0
...
$numberbase = 1
$base1name = 'Бухгалтерия предприятия 8.2'
$base1patch = 'D:\1C BASE 8.2\Buhpp82'

$base2name = 'Зарплата и управление персоналом 8.2'
$base2patch = 'D:\1C BASE 8.2\HRM82_2'

$base3name = 'Управление торговлей 8.2'
$base3patch = 'D:\1C BASE 8.2\Trade82'

$base4name = 'Управление производственным предприятием 8.2'
$base4patch = 'D:\1C BASE 8.2\Enterprise82'

run('"C:\Program Files (x86)\1cv82\common\1cestart.exe"')
while 1
	$thwnd='[TITLE:Запуск 1С:Предприятия;CLASS:V8NewLocalFrameBaseWnd]'
	if WinExists($thwnd)=1 Then
		_startFromStartWindowInstall()

	EndIf

		$thwnd='Добавление информационной базы/группы'
	if WinExists($thwnd)=1 Then
		switch $numberbase
			case 1
				$basename = $base1name
				$basepatch = $base1patch
			case 2
				$basename = $base2name
				$basepatch = $base2patch
			case 3
				$basename = $base3name
				$basepatch = $base3patch
			case 4
				$basename = $base4name
				$basepatch = $base4patch
		endswitch
;~ 		BlockInput(1)
				WinActivate($thwnd)
				controlclick($thwnd,'','[CLASS:V8FormElement; INSTANCE:10]')
				send('{Enter}')
				ConsoleWrite(_NowTime(5)&'(' & @ScriptLineNumber & ') ' &" new base" & @CRLF)
				sleep(500)
				Send ($basename)
				send('{Enter}')
				sleep(500)
...
func _startFromStartWindowInstall()
			 WinActivate($thwnd)
;~ 		 if timerdiff($timeoutStart) > 15000 then
;~ 			 $timeoutStart= Timerinit()
;~ 			if $Startchange = 1 then
;~ 				$CloseWinPp=0
				controlclick($thwnd,'','[CLASS:V8FormElement; INSTANCE:7]')
				send('{Enter}')
				ConsoleWrite(_NowTime(5)&'(' & @ScriptLineNumber & ') ' &" запуск установки 1С предприятия" & @CRLF)
;~ 			Else
