...
$hGui = GUICreate("Извлечение информации из отчетов AIDA",  610, 293, 192, 125, -1, BitOR($WS_EX_ACCEPTFILES,$WS_EX_WINDOWEDGE))
...
$Combo1 = GUICtrlCreateCombo("Не использовать список программ", 88, 40, 209, 25, BitOR($CBS_DROPDOWN,$CBS_AUTOHSCROLL))
GUICtrlSetData(-1, "Извлечь список программ|Извлечь и обработать список ПО")
...
Func Combo1Change()
$textCombo = GUICtrlRead($Combo1)
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : Ubound(GUICtrlRead($Combo1)) = ' & Ubound(GUICtrlRead($Combo1)) & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;~ ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : GUICtrlRead($Combo1) = ' & GUICtrlRead($Combo1,1) & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
EndFunc
...
Func _display_information()
	...
	_AllToArray('$LANip|$windows|$Antivirus|$processor|$core|$RAM|$ddr|$partitions' , 5)
...
Func _ListSoft_extraction()


	;Список установленного программного обеспечения на компьютере
	$ListSoft = StringRegExp ( $Text, "(?i)<TABLE WIDTH=100%>.*?Installed Programs.*?</TABLE><TABLE>(?s).*?(?-s)<TR><TD><TD><TD>(?s).*?(?-s)<TR><TD><TD><TD>(?s)(.*?)(?-s)</TABLE>", 2)
	if not @error then
		$ListSoftFile = FileOpen ( @ScriptDir&'/Списки_ПО/' & $filename & '.txt' , 8+2 )
		$ListSoft[1] = StringRegExpReplace($ListSoft[1],'<(?i)TD .*?>',@TAB)
		$ListSoft[1] = StringRegExpReplace($ListSoft[1],'<.*?>','')
		$ListSoft[1] = StringReplace($ListSoft[1],'&nbsp;',' ')
;~ 	ClipPut($ListSoft[1])
;~ _ArrayDisplay($ListSoft)
		ClipPut($ListSoft[1])
;~ 		 $t9 = Run(@AutoItExe & ' /AutoIt3ExecuteScript "' & @ScriptDir & '\Обработка_текста.au3"')
		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : StringLen($ListSoft[1]) = ' & StringLen($ListSoft[1]) & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		$t9 = RunWait('"'&@ScriptDir & '\Обработка_текста.exe"')
		 ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $t9 = ' & $t9 & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		if not @error then $ListSoft[1] = ClipGet()
		ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : StringLen($ListSoft[1]) = ' & StringLen($ListSoft[1]) & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		FileWrite($ListSoftFile, $ListSoft[1])
		FileClose ( $ListSoftFile )
;~ 		Return $ListSoft[1]
	EndIf
;~ 	Return ''
EndFunc

Func _AllToArray($String,$dimen = 1)
	Dim $TempArray[$dimen]
	$aString = StringSplit($String,'|')
	For $i=1 to Ubound($aString) -1
		if StringLeft($aString[$i],1) = '$' then $aString[$i]=StringTrimLeft($aString[$i],1)
		if IsArray(Eval($aString[$i])) = 0 then Assign ( $aString[$i], $TempArray )
		ConsoleWrite(' Переменной ' & $aString[$i] & ' была измененна на массив' & @CRLF)
	Next
EndFunc
...
$textCombo = GUICtrlRead($Combo1)
if $textCombo <> 'Не использовать список программ' then
	_ListSoft_extraction()
EndIf
