...
['WinRAR', 7 ], _
['WinUtilities', 8 ], _
['Vit Registry Fix', 8 ], _
['ESET ', 7 ], _
['FineReader', 7 ], _
['Foxit ', 4 ], _
['Unlocker', 4 ], _
['Easy Tune \E\d\Q B', 3 ], _
['AutoGreen B', 3 ], _
['Net Speakerphone', 2 ], _
['Live\E.*\QMicrosoft', 2 ]]
$SortString=_ArrayClearEmpty($SortString, 0, 0)


$fileLog = FileOpen("LogDelete.csv",9)
$date =  @MDAY & '/' & @MON & '/' & @YEAR & ' - ' & @HOUR & ':' & @MIN & ':' & @SEC
FileWriteLine($fileLog,' '&@CRLF& '____<<<' & $date & ' ' & _ListWindows())
...
;~ 	HotKeySet("{Ins}", "_testFun")
;~ 	Func _testFun()
;~ 	;~ 	$t1=GUICtrlSendMsg($ListView, $LVM_SETEXTENDEDLISTVIEWSTYLE, 0, $LVS_EX_CHECKBOXES + $LVS_EX_GRIDLINES)
;~ 	;~ 	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $t1 = ' & $t1 & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;~ 	;~ 	$t2=GUICtrlSendMsg($ListView, $LVM_SETEXTENDEDLISTVIEWSTYLE, 1, $LVS_EX_CHECKBOXES + $LVS_EX_GRIDLINES)
;~ 	;~ 	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $t2 = ' & $t2 & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;~ 	;~ 	$t3=GUICtrlSendMsg($ListView, $LVM_SETEXTENDEDLISTVIEWSTYLE, 2, $LVS_EX_CHECKBOXES + $LVS_EX_GRIDLINES)
;~ 	;~ 	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $t3 = ' & $t3 & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;~ 	$t6=_GUICtrlListView_SetExtendedListViewStyle($ListView, BitOR($LVS_EX_GRIDLINES, $LVS_EX_FULLROWSELECT, $LVS_EX_SUBITEMIMAGES, $LVS_EX_CHECKBOXES))
;~ 	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $t6= = ' & $t6 & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
;~ 	endfunc
...
;~ 				FileWriteLine($fileLog,
				$tempLine=$splitBuffer[_ArrayMaxIndex($tempArray,1,1)]
				FileWriteLine($fileLog, '>>>'&StringLeft($tempLine,StringInStr($tempLine,@TAB)))
				_textLog($splitBuffer[_ArrayMaxIndex($tempArray,1,1)])
				$tempLine=$splitBuffer[$i]
				$tempEqualDeleteList[$i]=1
				FileWriteLine($fileLog, '>'&StringLeft($tempLine,StringInStr($tempLine,@TAB)))
				_textLog($splitBuffer[$i])
			EndIf
		 Next
	  EndIf
  EndIf
...
For $i=1 to $SplitBuffer[0]
	if $tempEqualDeleteList[$i]<>1 then
		FileWriteLine($fileLog, StringLeft($splitBuffer[$i],StringInStr($splitBuffer[$i],@TAB)))
		_textLog($splitBuffer[$i])
	EndIf
...
For $i=1 to Ubound($SplitBuffer)-1
	if $splitBuffer[$i]<>'' then
		...
		For $k=0 to UBound($SortString)-1 ;расстановка цветов
			If StringRegExp ( $SplitBuffer[$i], '(?i)\Q'& $SortString[$k][0] &'\E') <>0 Then
				if $SortString[$k][1] >= 7 Then
					GUICtrlSetColor( -1, 0x0000FF)
				EndIf
				if $SortString[$k][1] <= 6 then
...
;Поиск не учтенных удалённых строк
For $i=1 to $CopySplitBuffer[0]
	if _ArraySearch($splitBuffer, $CopySplitBuffer[$i] ) > 0 then
;~ 		_ArrayDelete($CopySplitBuffer,$i)
		$CopySplitBuffer[$i]=''
	EndIf
Next
$CopySplitBuffer=_ArrayClearEmpty($CopySplitBuffer, 0, 1)
;запись в ListView вырезанных строк
For $i=1 to Ubound($CopySplitBuffer)-1
	GUICtrlCreateListViewItem(StringReplace($CopySplitBuffer[$i],@TAB,'|') ,$ListView)
	GUICtrlSetColor( -1, 0xFF0000)
Next
For $i=1 to Ubound($CopySplitBuffer)-1
	if _ArraySearch($aTextLog, $CopySplitBuffer[$i] ) > 0 then
;~ 		_ArrayDelete($CopySplitBuffer,$i)
		$CopySplitBuffer[$i]=''
	EndIf
Next
...
;занесение текста лога в массив
Func _textLog($TextLog)
	$tempdim = Ubound($aTextLog)
	Redim $aTextLog[$tempdim+1]
	$aTextLog[$tempdim] = $TextLog
EndFunc
