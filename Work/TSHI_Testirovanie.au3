...
Func _StepGUI($iNext = 1)
    ...
    If $iStep <= 0 Then ; Если номер окна равен 0 или меньше то сброс
        ...
        GUICtrlSetData($iQuestion, 'ТСХИ') ; Вступительный текст при возврате в начало
...
;~ 				Next
   $Table4 = _GUICtrlTable_Create(8, 32, 69, 32, 3, 8, 1)
   _GUICtrlTable_Set_ColumnWidth($Table4, 1, 100)
   
   _GUICtrlTable_Set_Text_Row($Table4, 1, "Характерист- ка|Ч-Человек|Ч-Техника|Ч-Знаковая система|Ч-Художественный образ|Ч-Природа|Исполнительский|Творческий")
  _GUICtrlTable_Set_Text_Cell($Table4, 2, 1, 'интересы и склонности')
   _GUICtrlTable_Set_Text_Cell($Table4, 3, 1, 'возможности')
   For $i = 1 To UBound($MassivOtvetov) - 1
;~ 				   MsgBox(4096, "флаг=2",   "$aArray[" & $i & "] = " & $aArray[$i])
				   _GUICtrlTable_Set_Text_Cell($Table4, 2, $i+1, $MassivOtvetov[$i])
				Next
